...
Для уже существующего direct chat история сообщений может оставаться доступной участникам, но write-операции чата
разрешены только пока между двумя участниками сохраняется активная friendship и отсутствует block хотя бы в одну
сторону.

Следствия:

- удаление friendship не удаляет direct chat автоматически;
- `SendTextMessage` должен отклоняться, если friendship больше не активна;
- `SendTextMessage` должен отклоняться, если один пользователь заблокировал другого;
- read/list операции текущего этапа не обязаны скрывать уже существующую историю только из-за разрыва friendship.
